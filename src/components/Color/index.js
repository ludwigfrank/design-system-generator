    ...
    <Category themeID={themeID} category={"Color"}>
        <View name={"contentWrapper"} style={{ marginTop: "24px" }}>
            {Object.keys(colorGroups).map(colorGroup => (
                <View name={"swatchGroup_" + colorGroup} key={colorGroup}>
                    <UITypo type="h3" themeID={themeID}>
                        {colorGroup.replace(/\b\w/g, l => l.toUpperCase())}
                    </UITypo>
                    <View
                        style={ColorSwatchWrapperStyle}
                        name="swatches"
                        key={colorGroup}
                    >
                        {Object.keys(colorGroups[colorGroup]).map(
                            (color, index) => (
                                <ColorSwatch
                                    themeID={themeID}
                                    color={
                                        colorGroups[colorGroup][color][themeID]
                                    }
                                    groupName={colorGroup}
                                    name={color}
                                    index={index}
                                    shouldRenderOutline={
                                        !!colorGroups[colorGroup][color].outline
                                    }
                                    shouldRenderStates={
                                        !!colorGroups[colorGroup][color].state
                                    }
                                    key={color}
